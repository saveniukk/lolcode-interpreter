...
def test_function_return():
    interp = Interpreter()
    prog = Program([
        FunctionDefinition("add", ["a", "b"], [
            ReturnStatement(BinaryOperation("SUM OF", Variable("a"), Variable("b")))
        ]),
        Assignment("result", FunctionCall("add", [Literal(2), Literal(3)])),
        VisibleStatement(Variable("result"))
    ])
...
def test_function_without_return_returns_none():
    interp = Interpreter()
    prog = Program([
        FunctionDefinition("no_return", [], [
            Assignment("x", Literal(5))
        ]),
        Assignment("result", FunctionCall("no_return", [])),
        VisibleStatement(Variable("result"))
    ])
    with patch("builtins.print") as mocked_print:
        interp.interpret(prog)
        mocked_print.assert_called_with(None)

def test_function_call_wrong_argument_count():
    interp = Interpreter()
    prog = Program([
        FunctionDefinition("one_arg", ["x"], [
            ReturnStatement(Variable("x"))
        ]),
        Assignment("res", FunctionCall("one_arg", [Literal(1), Literal(2)]))
    ])
